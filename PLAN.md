...
**Чего нет:** офлайн-кэша проектов. `FileProjectStore` написан и под
тестами, но приложением не используется — библиотека ходит в Supabase.
Кэш «на чтение» показывал бы строки, которые можно открыть и править, а
каждый сейв молча падал бы: человек думает, что сохранил. Честная
пустая библиотека офлайн — это ровно то, что делает веб. Двусторонняя
синхронизация — отдельная работа, не в этой вехе.
