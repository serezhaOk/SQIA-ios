...
### M5 — Голоса и эффекты (в работе: 2 из 5 пресетов)
Сделано: генераторы (8 форм Tone, saw/square band-limited через PolyBLEP),
ADSR и питч-огибающая, шум white/pink, хорус, пинг-понг-дилей, овердрайв
по кривой Tone, ревербератор (FDN), цепь пресета с потактовым дрейфом и
«даком» эха при переброске. Голоса REVERIE и MACHINE — с точным порядком
розыгрышей. Сэмплы удалены по решению владельца: список голосов теперь
только синты, `voiceIdx` совпадает с вебом (0…4).
Осталось: KALIMBA (Karplus-Strong), RHODES (FM + тремоло), ACID (303).
