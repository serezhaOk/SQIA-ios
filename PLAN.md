...
| Аудио-граф | Web Audio API | **AVAudioEngine** | Нативный эквивалент, sample-accurate планирование |
| Синтез (5 пресетов) | Tone.js | Собственные DSP-голоса на `AVAudioSourceNode` + AudioKit/SoundpipeAudioKit там, где узел совпадает 1:1 (Karplus-Strong, шум) | Точный контроль огибающих и рандомизации «как в Tone»; AudioKit целиком не тянуть — важнее повторить поведение, чем набор классов |
| Эффекты | Tone.Filter/Chorus/PingPongDelay/Reverb/Distortion | Кастомные/AudioKit-узлы; ревербератор — см. риск R3 | |
...
| Зависимости | supabase-js, tone | SPM: supabase-swift; точечно AudioKit/SoundpipeAudioKit | Минимум |
...
- Снапшот проекта (JSON в `projects.tracks`, **формат байт-в-байт как в
  вебе** — иначе сломаем кроссплатформенность):
  `{bpm, root_pc, scale, tracks:[{voiceIdx, muted, cells[192]}]}`,
  cells округлены до 2 знаков.
...
- пример снапшота проекта (байтовое сравнение JSON после нормализации);
...
### M0 — Бутстрап (1–2 дн)
Xcode-проект, SPM (supabase-swift, точечно AudioKit), таргет iOS 17,
портрет; импорт ассетов: иконки приложения из `public/icon-*.png`,
шрифты, 16 сэмплов; чёрный launch screen; CI на GitHub Actions
(macos-runner: build + unit-тесты); базовый lint.
**Приёмка:** пустое приложение собирается в CI, шрифты рендерятся.

### M1 — Музыкальное ядро + фикстуры (2–3 дн)
Порт `scales.ts`, `Grid` (модель без рендера), `RandomNames`,
`ProjectSnapshot` Codable с точным JSON; `tools/gen-fixtures`;
юнит-тесты на golden-данных.
**Приёмка:** все фикстуры зелёные; JSON снапшота после нормализации
байт-в-байт совпадает с вебом.

### M2 — Аудио-фундамент (3–4 дн)
AVAudioEngine-граф: мастер/лимитер/слап-дилей; StepTransport
(лукахед 25/120 мс, BPM 40–240, смена темпа на лету); AVAudioSession
(.playback, обработка прерываний звонком, смены маршрута, свернули —
пауза, вернулись — продолжили); загрузка/кэш сэмплов из бандла;
one-shot-плеер с огибающей и rate.
**Приёмка:** метроном-паттерн на сэмпле держит темп без дрейфа 10 мин,
переживает звонок и AirPods.

### M3 — Рендер сцены на Metal (5–7 дн)
Порт математики grid.ts (warp/unwarp/fitCell/push/waves/breathe) в
GridRenderer; Grid.metal: инстансы точек, атлас глоу, стрики,
additive blending; параметры detail/alpha; 60/120 fps.
**Приёмка:** видео-сравнение бок о бок с вебом (то же поле, тот же
паттерн) неразличимо; Instruments: стабильный fps на iPhone 12 и SE.

### M4 — Экран секвенсора + жесты (4–6 дн)
SequencerView: шапка/футер по метрикам CSS; кисть по дробным
координатам, ластик, RNDM, BPM-драг/тап, корень/лад, метка голоса;
onStep → вспышки/плейхед с задержкой до звучания; вертикальный срез
с одним простым голосом.
**Приёмка:** «рисуешь — звучит» с точным совпадением поведения кисти
(golden-тесты) и всех жестов по списку 2.1.3.
...
| R3 | Ревербератор: в вебе — свёртка с генерируемым IR (7 с), на iOS готового convolution-узла нет | Шаг 1: алгоритмический (AVAudioUnitReverb/Costello), тюнинг на слух; шаг 2 (если A/B заваливается): partitioned-FFT-свёртка на vDSP с тем же генерируемым IR (+2–3 дн, заложено в верхнюю границу M5) |
